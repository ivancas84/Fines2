...
use FinesApp\Repositories\ComisionesPfImportRepository;

final class ComisionesPfController extends Controller
{
    public function form(Request $request, array $vars = []): void
    {
        ...
        $this->view->render('comisiones/procesar_pf', [
            'title' => 'Procesar comisiones PF',
    ...
    public function process(Request $request, array $vars = []): void
    {
        ...
        try {
            if (trim($rawData) === '') {
                throw new \InvalidArgumentException('Pegá los datos copiados del informe global.');
            }
            if ($selectedCalendario === '') {
                throw new \InvalidArgumentException('Seleccioná un calendario.');
            }

            $report = (new ComisionesPfImportRepository($this->pdo))->process(
                $rawData,
                $selectedCalendario,
            );

            $notice = sprintf(
                'Proceso finalizado: %d líneas, %d cursos, %d horarios, %d tomas creadas, %d conflictos, %d errores.',
                (int) $report['lines_total'],
