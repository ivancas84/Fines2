...
namespace SqlOrganize.Sql.Fines2Model3
{
    internal static class ProgramaFines
    {
        ...
        {
            HttpResponseMessage response = await client.GetAsync("https://programafines.ar/");
            response.EnsureSuccessStatusCode(); // Ensure a successful response

            // Prepare the form data to be sent in the POST request
            var formData = new Dictionary<string, string>
                    {
                        { "usuario", (Context.db.config as Config).pfUser },
                        { "password", (Context.db.config as Config).pfPassword },
            ...
            response = await client.PostAsync("https://programafines.ar/validar.php", encodedFormData);
                response.EnsureSuccessStatusCode(); // Ensure a successful response
        ...
        public static async Task PF_InscribirEstudianteValues(HttpClient client, string comisionPfid, Entity persona)
        {
            Dictionary<string, string> dataForm = new();

            dataForm["nombre"] = persona.Get("nombres")?.ToString() ?? "";
        ...
        {
            client.Dispose();
            Context.pfHandler.Dispose();
            Context.pfHandler.CookieContainer = new CookieContainer();
        }
