...
namespace Fines2Model3.Data
{
    public class Data_detalle_persona_r : Data_detalle_persona
    {
        ...
        public Data_detalle_persona_r (Db db) : base(db)
        {
        }

        public Data_detalle_persona_r (Db db, params string[] fieldIds) : this(db)
        {
            Init(fieldIds);
        }
        ...
        protected void Init(params string[] fieldIds)
        {
            foreach(string fieldId in fieldIds)
            {
                ...
                {
                    case "archivo":
                        archivo__id = (string?)db!.Values("file").GetDefault("id");
                        archivo__created = (DateTime?)db!.Values("file").GetDefault("created");
                    break;
                    case "persona":
