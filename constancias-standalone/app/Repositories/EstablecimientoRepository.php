...
final class EstablecimientoRepository
{
    ...
    public function saveForUser(int $userId, array $data): int
    {
        ...
        if ($current === null) {
            $stmt = $this->pdo->prepare("
                INSERT INTO fines_app_establecimientos (nombre, cue, direccion, logo_path, firma_director_path, sello_oval_path)
                VALUES (:nombre, :cue, :direccion, :logo_path, :firma_director_path, :sello_oval_path)
            ");
            $stmt->execute([
                'nombre' => $data['nombre'],
                'cue' => $data['cue'] ?: null,
