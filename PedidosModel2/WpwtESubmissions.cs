...
{
    public class WpwtESubmissions : SqlOrganize.Sql.EntityData
    {

        public override string entityName => "wpwt_e_submissions";

        public override void Default()
        {
            EntityVal val = db!.Values("wpwt_e_submissions");
            ...
            switch (columnName)
            {
                ...
                case "hash_id":
                    ...
                    if (!db.IsNoE() && !_hash_id.IsNoE()) {
                        var row = db.Sql("wpwt_e_submissions").Equal("$hash_id", _hash_id).Cache().Dict();
